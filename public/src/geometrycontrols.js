...
GeometryControls.prototype.bindInfoWindow = function(geomInfo){
  ...
  var startInput = get$(cssId + "-start");
  var endInput = get$(cssId + "-end");
  ...
  titleInput.value = (title[1]) ? ((title[0] === title[1]) ? title[0] : title[1]) : ""; //return the saved data or most recent edit
	startInput.value = (start[1]) ? ((start[0] === start[1]) ? start[0] : start[1]) : "";
	endInput.value = (end[1]) ? ((end[0] === end[1]) ? end[0] : end[1]) : "";
  GEvent.addDomListener(titleInput,"change", function(){
    title[1] = titleInput.value;
  });
  GEvent.addDomListener(startInput,"change", function(){
    start[1] = startInput.value;
  });
  GEvent.addDomListener(endInput,"change", function(){
    end[1] = endInput.value;
  });
